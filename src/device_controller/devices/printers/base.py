...
from validators.response import ResponseTypes, DeviceResponse, PrinterResponse
...
class BasePrinterDriver(BaseDeviceDriver):
    ...
    async def two_ways_exchange(self, host: str, port: int, websocket_id: str) -> DeviceResponse:
        ...
        try:
            ...
            response_decoded: PrinterResponse | None = self.decode_response_func(response)
    ...
    async def send(self, host: str, port: int, websocket_id: str, dpl_command: str) -> DeviceResponse:
        ...
        try:
            ...
            # Возвращаем серверу ответ
            return DeviceResponse(device=(host, port), ok=True, type=ResponseTypes.info, data=None, message=None)
