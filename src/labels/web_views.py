...
async def web_get_all_labels(
    ...
    all_labels = await labels_service.get_all(session)
...
async def web_create_label_form(
    ...
    context = await labels_service.create_form(session)
...
async def web_update_label_form(
    ...
    context = await labels_service.update_form(session, label_id)
...
async def web_create_label(
    ...
    label_id = await labels_service.create(session, name, driver_name, print_command)
...
async def web_update_label(
    ...
    """Сохраняем изменения шаблона этикетки."""
    await labels_service.update(session, label_id, name, driver_name, print_command)
