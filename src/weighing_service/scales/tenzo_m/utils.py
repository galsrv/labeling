...
def _compute_crc(data: bytes, as_hex: bool = False):
    """
...
def decode_weight_frame(data: bytes) -> tuple | None:
    """
    ...
    try:
        ...
        W0, W1, W2, CON = core_without_crc[2], core_without_crc[3], core_without_crc[4], core_without_crc[5]
...
def generate_random_weight_response(min_kg: float, max_kg: float) -> bytes:
    ...
    W0 = to_bcd_pair(weight_str[4:6])
    W1 = to_bcd_pair(weight_str[2:4])
    W2 = to_bcd_pair(weight_str[0:2])

    # --- 3️⃣ Формируем байт CON ---
    sign = 0 if weight_kg >= 0 else 1
    stable = random.choice([0, 1])
    overload = 1 if random.random() < 0.05 else 0
    CON = (sign << 7) | (stable << 4) | (overload << 3) | decimals
    ...
    CRC = _compute_crc(core)  # важно: теперь CRC рассчитывается для Adr..CON
