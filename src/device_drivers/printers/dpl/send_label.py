from device_drivers.printers.dpl.control_codes import get_control_codes
...
def build_dpl_unicode_label(data: str, control_codes: int = 0) -> bytes:  # noqa: C901
    ...
    # Treat line breaks and <CR> as the same record separator
    data = data.replace("\r\n", "\n").replace("\r", "\n")
    data = data.replace("\n", "<CR>")
    # Collapse duplicate separators (e.g., <CR> + newline)
    while "<CR><CR>" in data:
        data = data.replace("<CR><CR>", "<CR>")
    data = data.replace("_", "")
